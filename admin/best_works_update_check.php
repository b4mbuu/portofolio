...
if(isset($_POST['submit'])){
	...
	if(!empty($works_name) && !empty($catagory) ){
		...
		if($work_insert){
			...
			$_SESSION['best_works_update_success'] = "Амжилттай шинэчлэв!";
...
if(isset($_POST['photo_submit'])){
	...
	if(!empty($photo)){
		...
		if($_FILES['photo']['size']<5000000){
			...
			if( in_array(strtolower($photo_ext),$accepted_extension_list) ){
				...
				if($update_photo){
					...
					$_SESSION['best_works_photo_success'] = "Зургийг амжилттай хавсаргав!";
				...
				$_SESSION['best_works_photo_extension'] = "jpg,png эсвэл jpeg зураг хавсаргана уу";
				header('location: best_works_update.php?id=' . base64_encode($id));
				ob_end_flush();
			}
			...
			$_SESSION['best_works_photo_size'] = "1MB-аас бага хэмжээтэй зураг хавсаргав";
			header('location: best_works_update.php?id=' . base64_encode($id));
			ob_end_flush();
		}
		...
		// if empty
		$_SESSION['best_works_photo_emty'] = "Зураг хавсаргав";
		header('location: best_works_update.php?id=' . base64_encode($id));
		ob_end_flush();
	}
